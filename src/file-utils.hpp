...
list_files(const std::string &parent_path, fileTypes types)
{
	...
	{
		for (auto &entry_path : boost::make_iterator_range(
				bfs::recursive_directory_iterator(parent_path)))
		{
	...
	log_state << list_of_files.size() << " " << object_name << " in " << parent_path
	          << " found" << std::endl;
